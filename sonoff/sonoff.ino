...
void SetLatchingRelay(power_t lpower, uint8_t state)
{
  ...
  { // Set latching relay to power if previous pulse has finished
...
void SetDevicePower(power_t rpower, int source)
{
  ...
  { // Set power state and stop if serviced
    // Serviced
  }
...
void SetLedPowerIdx(uint8_t led, uint8_t state)
{
  ...
  { // Legacy - LED1 is link led only if LED2 is present
    if (pin[GPIO_LED2] < 99)
    {
      led = 1;
    }
...
void SetLedPower(uint8_t state)
{
  ...
  { // Legacy - Only use LED1 and/or LED2
...
bool SendKey(uint8_t key, uint8_t device, uint8_t state)
{
  ...
  if (Settings.flag.mqtt_enabled && MqttIsConnected() && (strlen(key_topic) != 0) && strcmp(key_topic, "0"))
  {
    ...
    {
      device = 1;
    } // Only allow number of buttons up to number of devices
...
void ExecuteCommandPower(uint8_t device, uint8_t state, int source)
{
  ...
  if (state <= POWER_TOGGLE)
  {
    ...
    { // Clear all but masked relay in interlock group
      interlock_mutex = true;
      for (uint32_t i = 0; i < MAX_INTERLOCKS; i++)
      {
        if (Settings.interlock[i] & mask)
        { // Find interlock group
          for (uint32_t j = 0; j < devices_present; j++)
          {
            power_t imask = 1 << j;
...
void Every250mSeconds(void)
{
  ...
  { // Problem blinkyblinky enabled
    if (global_state.data)
    { // Any problem
      if (global_state.mqtt_down)
      {
        blinkinterval = 7;
      } // MQTT problem so blink every 2 seconds (slowest)
      if (global_state.wifi_down)
      {
        blinkinterval = 3;
      }             // Wifi problem so blink every second (slow)
  ...
  if (blinks || restart_flag || ota_state_flag)
  {
    ...
    {                    // Overrule blinks and keep led lit
...
\*-------------------------------------------------------------------------------------------*/
    ...
    { // Discard binary data above 127 if no raw reception allowed
      serial_in_byte_counter = 0;
      Serial.flush();
      return;
    }
    ...
    else
    {
      ...
      {                                                                                               // Any char between 1 and 127 or any char (0 - 255)
...
void GpioInit(void)
{
  ...
  if ((2 == pin[GPIO_TXD]) || (H801 == my_module_type))
  {
    Serial.set_tx(2);
  }
...
void setup(void)
{
  ...
  if (VERSION & 0xff)
  { // Development or patched version 6.3.0.10
  ...
  { // Allow setting override only when all is well
